    ...
    StrategyManagerStorage
{
    ...
    modifier onlyDelegationManager() {
        require(
            ...
            "StrategyManager.onlyDelegationManager: not the DelegationManager"
        );
        _;
    }
    ...
    function depositIntoStrategyWithSignature(
        ...
        bytes32 structHash = keccak256(
            abi.encode(DEPOSIT_TYPEHASH, staker, strategy, token, amount, nonce, expiry)
        );
        unchecked {
    ...
    function addStrategiesToDepositWhitelist(
        ...
        for (uint256 i = 0; i < strategiesToWhitelistLength; ) {
            ...
            if (!strategyIsWhitelistedForDeposit[strategiesToWhitelist[i]]) {
                ...
                _setThirdPartyTransfersForbidden(
                    strategiesToWhitelist[i],
                    thirdPartyTransfersForbiddenValues[i]
                );
            }
            unchecked {
                ++i;
