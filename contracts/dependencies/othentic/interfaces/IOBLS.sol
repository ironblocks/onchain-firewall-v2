...
import {BLSAuthLibrary} from "../BLSAuthLibrary.sol";
...
interface IOBLS {
    ...
    function increaseBatchOperatorVotingPower(
        OperatorVotingPower[] memory _operatorsVotingPower
    ) external;

    function increaseOperatorVotingPowerPerTaskDefinition(
        uint16 _taskDefinitionId,
        uint256 _votingPower
    ) external;
    ...
    function decreaseBatchOperatorVotingPower(
        OperatorVotingPower[] memory _operatorsVotingPower
    ) external;

    function decreaseOperatorVotingPowerPerTaskDefinition(
        uint16 _taskDefinitionId,
        uint256 _votingPower
    ) external;
